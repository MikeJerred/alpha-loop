...
export const getCompoundPositions = async (ethereum: EIP1193Provider) => {
  const results = await Promise.all(markets.map(async ([[chain, rpcUrls], baseToken, address]) => {
    ...
    const client = createPublicClient({
      chain,
      transport: fallback(rpcUrls.map(url => http(url, { batch: { batchSize: 8, wait: 150 } }))),
    });
    ...
    const [borrowAddress, borrowPriceFeed, borrowScale] = await Promise.all([
      comet.read.baseToken(),
      comet.read.baseTokenPriceFeed(),
      comet.read.baseScale(),
    ]);
    ...
    const supplied = (await Promise.all([...new Array(assetCount)].map(async (_, i) => {
      ...
      const amount = await comet.read.collateralBalanceOf([userAddress, address]);
